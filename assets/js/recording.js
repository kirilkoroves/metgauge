    ...
    reader.onload = (e) => {
        ...
        fetch(base64URL)
          ...
          .then(blob => {
            ...
            request.open("POST", "/api/convert_speech_to_text", true);
             request.onreadystatechange = () => {
                if (request.readyState === 4 && request.status === 200) {
                  $(".send_message_input").val(JSON.parse(request.responseText).text);
                }
                $(".send_message_btn").removeAttr("disabled");
                $(".start-recording-button").removeAttr("disabled");
                $(".send_message_input").removeAttr("disabled");
                $(".speech-dots").addClass("hidden");
            };
            const file = new File([blob], "current_wav.wav",{ type: "audio/x-wav" })
            data.append("file", file);
            data.append("_csrf_token", window.csrfToken);
            $(".send_message_btn").attr("disabled", "disabled");
            $(".start-recording-button").attr("disabled", "disabled");
            $(".send_message_input").attr("disabled", "disabled");
            $(".speech-dots").removeClass("hidden");
            request.send(data);
